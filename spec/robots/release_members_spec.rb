...
describe Robots::DorRepo::Release::ReleaseMembers do
  ...
  it 'runs the robot for an item or an apo and do nothing as a result' do
    %w[:item :apo].each do |item_type|
      ...
      expect(@r).not_to receive(:item_members) # we won't bother looking for item members if this is an item
      @r.perform(@work_item)
    end
  end

  it 'runs for a collection but never add workflow or ask for item members when the collection is released to self only' do
  ...
  it 'runs for a collection but never add workflow or ask for item members when there are multiple targets but they are all released to self only' do
  ...
  it 'runs for a collection and execute the item_members method when the collection is not released to self' do
  ...
  it 'runs for a collection and execute the item_members method when there are multiple targets and at least one of the release tagets is not released to self' do
    members = { 'items' => [{ 'druid' => 'druid:bb001zc5754', 'latest_change' => '2014-06-06T05:06:06Z', 'title' => 'French Grand Prix and 12 Hour Rheims: 1954', 'catkey' => '3051728' },
                            { 'druid' => 'druid:bb023nj3137', 'latest_change' => '2014-06-06T05:06:06Z', 'title' => 'Snetterton Vanwall Trophy: 1958', 'catkey' => '3051732' },
                            { 'druid' => 'druid:bb027yn4436', 'latest_change' => '2014-06-06T05:06:06Z', 'title' => 'Crystal Palace BARC: 1954', 'catkey' => '3051733' },
                            { 'druid' => 'druid:bb048rn5648', 'latest_change' => '2014-06-06T05:06:06Z', 'title' => '', 'catkey' => '3051734' }] }
  ...
  it 'runs for a collection and execute the sub_collection method' do
    ...
    expect(Dor::Release::Item).to receive(:add_workflow_for_collection).twice # two workflows added, one for each collection
    @r.perform(@work_item)
  end
  ...
  it 'runs for a collection and execute the sub_collection method but not add a workflow for the collection itself' do
    ...
    expect(Dor::Release::Item).to receive(:add_workflow_for_collection).twice # only two workflows added (it doesn't add a workflow for itself)
    @r.perform(@work_item)
  end
end
